...
$sql = "SELECT mcode, reconciliation.status AS recon, SUM(interest) AS interest, SUM(amount) AS amount, SUM(debt) AS debt, month, services.service AS service, count(transactions.service) AS ss FROM `transactions`JOIN months ON months.mcode =  Month(transactions.aproved_date) LEFT JOIN services ON transactions.service = services.id LEFT JOIN reconciliation ON months.recon = reconciliation.id WHERE transactions.status = 2 group by transactions.service order by mcode asc;";
...
?>
	...
	<td>K<?php echo number_format($row['interest'],2);?></td>
